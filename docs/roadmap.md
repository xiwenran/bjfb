...
> 状态说明：✅ 已完成 / 🚧 进行中 / ⏸ 待启动
> 最后更新：2026-06-15（完成同账号 6 小时发布频控硬拦，commit d6723dc）

---

## ✅ 2026-06-15 — 同账号 6 小时发布频控硬拦（commit d6723dc）

- **目标**：同一平台、同一账号，两篇笔记间隔必须至少 6 小时；上传前、调度生成、发布执行前都要硬拦。
- **涉及文件**：`scripts/skill_upload.py`、`src/publish-guard.js`、`src/scheduler-allocator.js`、`src/scheduler.js`、相关测试。
- **验收标准**：
  - records 上传校验能挡住同平台同账号 6 小时内重复排期。
  - 排期生成默认按 6 小时过滤同账号时间槽，排不下时进入未安排。
  - 调度器发布前按历史账本和本轮队列做最后频控，错排记录不会提交到平台。
  - 针对性测试覆盖上传校验、排期、调度守门。
