...
- **一句话现状**：上线后新增记录已接入主题索引、排期前确认、同账号 361 分钟硬约束和固定 seed 分层随机排期。
- **阻塞**：无；全量测试中的 `tests/account-mapping.test.js` 既有 1 项失败已确认与本专项无关，专项验收时单独列明，不把它计作本轮回归。
- **最近验证**：2026-07-15，必要边界与相关测试 34/34 通过，Python/Node 语法通过；全量 79/80，唯一失败为既有账号映射基线问题。
- **commit hash**：`db5d420`（当前安装版本；功能实现收口 `f0f291a`，必要回归测试 `db5d420`）

> 本文件记录知发项目所有阶段的规划与执行状态。
> 状态说明：✅ 已完成 / 🚧 进行中 / ⏸ 待启动
> 最后更新：2026-07-11（正文硬边界补丁 ✅：SYSTEM_PROMPT 新增「正文硬边界」四条 + 双端强制粘贴规则原文条款，commit eccb2a7）
