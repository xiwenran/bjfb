...
### [2026-09-09] 发布图片解码与笔记文案优化
- 当前阶段: 🚧 源码已提交，待应用打包切换与文案小样确认
- 一句话现状: 已增加真实图片解码与请求内复用，普通号和引流号统一完整文案及排版校验，上传 Skill 冲突已修正。
- 阻塞: 无代码同步阻塞；teacher 两份引流文案规则已同步并通过独立审查。用户自行打包，知发 UI 由 Claude Code 另行修改；真实批量文案未验收
- 最近验证: 图片接口 1/1、文案 Node 22/22、Python 5/5；隔离提交候选通过相同测试；Node/Electron 加载通过；Terra 独立审查及一次窄复核通过。真实 provider、飞书和平台发布未测，BMP/HEIC 支持有运行时边界。
- commit hash: `91695b9`（实现）；`6741d9d`（阶段起点）；确认方案已追加 Obsidian 知发方案文档
...
### [v2.6] 排期职责重划：求解拆出软件，软件只做校验  [详情 →](roadmap/pending/schedule-allocator-hang.md)
- 当前阶段: 🚧 施工中，冷眼审查已出结论（有条件通过），阻断问题集中修复中
- 一句话现状: 删除 422 行 DFS 求解器，`/api/import/schedule` 改为 fail-closed 校验器；求解移到 `scripts/schedule_allocator.py` 构造式实现，168 池/56 槽位实测 1.9ms（原为卡死）
- 阻塞: 无
- 最近验证: 端到端握手通过（Python 产出经 JS 校验器校验 56 条合规）、篡改数据能被拒收、全量测试 110 pass / 1 既有基线 fail；冷眼审查发现 2 个阻断 + 4 个硬约束绕过，已实跑复现并派修复
- commit hash: 缺（尚未 commit，修复完成并窄复核后提交）

---

### [稳定性] 非图片素材入口拦截
- 当前阶段: 🚧 源码已提交并打包，等待用户覆盖安装并确认运行版本
- 一句话现状: ExFAT 外置盘自动生成的 AppleDouble 旁车文件被外部脚本当成图片提交，蚁小二拒收致 10 篇推送失败；已在 create-records 入口加文件头校验，并让 C1 放行已确认失败的任务
- 阻塞: 用户退出旧进程并安装 `dist/mac-arm64/知发.app`，确认 UI 左下角显示 `7f4398a`（当前运行 `f916f5a`）
- 最近验证: 飞书 59 条记录 875 个垃圾附件已清除、复核 0 残留；两个导入测试全绿（既有失败 skill_upload gates scheduling 与本轮无关，已用 worktree 在改动前版本验证）；独立冷眼审查 5 条问题已修完
- commit hash: `7f4398a`
