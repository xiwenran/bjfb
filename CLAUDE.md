...
| POST | `/api/service/start` | 启动定时服务 |
| POST | `/api/service/stop` | 停止定时服务 |
...
| `publishRecords(records, reason)` | 实际发布，有 `finally` 保证 `publishing=false` |
| `publishSpecificRecord(recordId)` | 立即发布指定记录，先取消该记录定时任务 |
