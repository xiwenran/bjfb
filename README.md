...
- 同一平台、同一账号的相邻笔记必须间隔至少 361 分钟，此限制不可跳过。
- 同一店铺组内，不同账号发布相同具体主题时，会在生成排期前一次性提醒；可自动错开、调整时间窗或确认继续。
- 批量排期在时间窗内按固定 `seed` 分层随机，同一批次不会集中在同一分钟；相同输入可复算出相同结果。
- “早上 9 点左右”按 08:00—12:00 处理。时间容量不足时会明确失败，不会少排、改日期或压缩间隔。
- 主题间隔只处理功能上线后新增记录，旧记录不回填；主题索引、飞书或发布历史读取异常时停止排期。
...
- **修复标题/正文为空**：飞书公式字段（引用 AI 生成输出结果）在 API 中有时以 `{value: "内容"}` 或 `{value: [{text:...}]}` 格式包装返回，原来的 `getText` 没有处理这种格式，导致读取结果为空。现在兼容所有已知包装格式。
- **修复标签乱码**：`getTagValues` 遇到 `{value: [...]}` 对象时，原来的 `String(field.value)` 会输出 `[object Object]`，现在正确展开数组后再拆分标签。
