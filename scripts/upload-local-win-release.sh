...
ARTIFACT_PATH="${1:-dist/知发 Portable.exe}"

if [ ! -f "$ARTIFACT_PATH" ]; then
  echo "未找到本地 Windows 安装包：$ARTIFACT_PATH"
  echo "请先执行 npm run dist:win"
  exit 1
fi
...
DEFAULT_TAG="local-win-${CURRENT_BRANCH}-$(date +%Y%m%d-%H%M%S)"
RELEASE_TAG="${2:-$DEFAULT_TAG}"
RELEASE_TITLE="${3:-知发 Windows 本地构建 ${CURRENT_BRANCH} ${CURRENT_SHA}}"
RELEASE_NOTES="${4:-知发本地 Windows 打包上传。分支：${CURRENT_BRANCH}，提交：${CURRENT_SHA}。}"
