...
> 本文件供 Codex 在每次会话开始时快速恢复上下文。请在每次重要变更后同步更新。

---

## 语言

- 始终用简体中文回复，包括思考和分析；代码、命令、文件名、URL、专有名词保持原样。
- 工具输出、报错、git log 是什么语言，不影响回复语言；只有用户明确说「用 X 语言」才切。
- 发现自己在用非中文输出实质内容，下一条回复立刻切回中文。
...
### 标题/正文/标签撰写的唯一规则源头（2026-07-11 补）

任何 Agent（Codex / Claude / 子代理）替流水线撰写标题、正文、标签前，必须先用 `sed -n '/^const SYSTEM_PROMPT/,/;$/p' ~/zhifa/src/ai-writer.js` 把 SYSTEM_PROMPT 字面文本提取并**完整粘贴**进工作上下文，按其中「正文写法 + 正文硬边界」执行。不允许只引用路径，不允许自行转述或维护简化版——这套规则只有 ai-writer.js 一个源头。背景：2026-07-11 数学/英语批次 96 条正文用「复述标题 + 图片张数」凑行数，全部返工，根因是执行端未加载规则原文。
...
### 笔记发布目录扫描口径（2026-06-07 补）

- 扫描 `~/Downloads/26春教师项目/已制作/笔记发布` 时，当前扫描到的 13 个条目是“分类下的 PPT 主题”，不是最终笔记篇数。
- 真正的一篇待发布笔记，是每个 PPT 主题文件夹下面的下一级子文件夹；不要按 PPT 主题数或图片数量计算篇数。
- 这些真实笔记归属于账号分组，后续 zhifa-upload / zhifa-pipeline 处理时必须保留“账号分组 → PPT 主题 → 笔记子文件夹”的层级关系。
- 若 PPT 主题目录根部有直接内容图/视频，同时下级子文件夹也有素材，属于混合结构歧义，必须停下询问用户，不静默猜测。
- 上传前预览要按“时间段 → 账号”交叉分组展示，账号下面列该时间段要发的笔记，避免只给按时间排序的流水表。
- `skill_upload.py create` 必须输出完整上传结果 JSON，供 `postprocess` 和审查复用；不得再临时手写上传脚本只为保存返回值。
- 排期器里同一素材笔记可跨平台复用：小红书用过的 `noteKey` 仍可给抖音使用；只禁止同一平台内重复使用同一 `noteKey`。
