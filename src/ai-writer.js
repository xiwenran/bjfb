...
5. 预习、复习、衔接、暑假、寒假、假期、开学摸底等学段/时段场景词，只有笔记主题中明确包含对应词（预习/复习/衔接/暑假/寒假/假期）时，标题（搜索词层）和正文才可以使用
6. 主题未包含任何场景词时，一律按"老师备课课件/日常教学资料"方向撰写，不得根据季节、日期或想象自创使用场景（如凭当前季节自行判断"现在是暑假，就往暑假预习方向写"）

> 说明：钩子层封闭菜单⑤"衔接课备课不愁"、⑧"晨读自习能用上"说的是"老师使用这份资料的场景/收益"，不是给资料本身定性为"预习/复习/衔接资料"，因此钩子层可以照常使用，不受第5-6条限制；第5-6条限制的是标题**搜索词层**（系列词/资料词/细分主题词）和**正文**里出现场景词。
...
// 场景词表：只有笔记主题明确包含对应词，标题搜索词层/正文才允许出现——防止 AI 按季节/日期自创使用场景（如"暑假预习"）
// 不含"衔接"：钩子层封闭菜单⑤"衔接课备课不愁"是允许保留的固定钩子文案，若把"衔接"也纳入场景词表会持续误伤该钩子
// 本表用于标题搜索词层的裸词检查；正文层的检查口径见 DESC_SCENE_TIME_WORDS / DESC_SCENE_QUALIFIER_REGEX
const SCENE_WORDS = ['预习', '复习', '暑假', '寒假', '假期'];

// 取标题的"搜索词层"（第一个全角逗号之前的部分）。场景词检查只查这一层，钩子层（逗号之后）豁免——
// 因为封闭钩子菜单里的固定文案（如⑤"衔接课备课不愁"）不应被场景词检查误伤。
function getTitleSearchLayer(title) {
  const idx = String(title || '').indexOf('，');
  return idx >= 0 ? title.slice(0, idx) : String(title || '');
}

// 正文层裸词检查只保留时段词；"预习/复习"只查强定性搭配（见 DESC_SCENE_QUALIFIER_REGEX）
const DESC_SCENE_TIME_WORDS = ['暑假', '寒假', '假期'];
// 正文层"预习/复习"只匹配把资料本身定性为预习/复习资料的搭配——
// "复习导入""带学生预习课文"这类教学动作描述不算越权
const DESC_SCENE_QUALIFIER_REGEX = /(预习|复习)(笔记|资料|讲义|清单|专用|合集)/g;

// 检查场景词越权：标题搜索词层或正文里出现场景词，但笔记主题不含该词 → 违规
// 标题搜索词层：裸词全表；正文：时段词裸词 + "预习/复习"仅强定性搭配
function checkSceneWordOverreach(title, description, topic) {
  const violations = [];
  if (!topic) return violations; // topic 未传入时跳过本检查（向后兼容）
  const topicStr = String(topic);
  const searchLayer = getTitleSearchLayer(title);

  // 收集正文中命中的场景词
  const descHits = new Set();
  for (const word of DESC_SCENE_TIME_WORDS) {
    if (description.includes(word)) descHits.add(word);
  }
  DESC_SCENE_QUALIFIER_REGEX.lastIndex = 0;
  let m;
  while ((m = DESC_SCENE_QUALIFIER_REGEX.exec(description)) !== null) {
    descHits.add(m[1]); // m[1] 是"预习"或"复习"
  }

  for (const word of SCENE_WORDS) {
    if (topicStr.includes(word)) continue; // 主题本身包含该词，允许使用
    const inTitle = searchLayer.includes(word);
    const inDescription = descHits.has(word);
    if (inTitle || inDescription) {
      const where = inTitle && inDescription ? '标题和正文' : (inTitle ? '标题' : '正文');
      violations.push(`场景词「${word}」越权：主题未包含「${word}」，不得在${where}中使用（主题=${topicStr}）`);
    }
  }
  return violations;
}
...
// topic 为可选参数：传入时额外做场景词越权检查（主题=xxx 时标题/正文才能出现"预习/复习/暑假/寒假/假期"），不传则跳过
function validateGenerated(content, platform, topic) {
  const violations = [];
  ...
  // ── 场景词越权（可选，传入 topic 时才检查） ──
  violations.push(...checkSceneWordOverreach(title, description, topic));
...
async function generateContent(aiConfig, record) {
  ...
  let violations = validateGenerated(parsed, platform, record.topic);
  ...
  if (violations.length > 0) {
    ...
    violations = validateGenerated(parsed, platform, record.topic);
    if (violations.length > 0) {
