...
**例外**：模型可运用自身对该课文、教材的教学知识合理展开（如描述《母鸡》的重点段落），不算编造。

**B. 家长会/班主任经验/育儿/班级管理类**（无课文名，以教师经历或沟通场景为主）：
直接写出核心观点和具体内容，不要写成「聊了X、围绕Y展开」这种会议纪要式总结。
允许模型根据教育场景合理延伸（如「五年级语文是爬坡期」可展开写阅读/作文/基础的具体变化），用教育专业知识把主题里的观点说实、说透。
语言克制、不煽情，写清楚有什么、为什么、怎么做就行。

**C. 备课资料/课件资源类**（有课文名或资料说明，以分享资料为主要场景）：
列出资料包含的主要模块，说明适用场景，语言简洁。
...
正确示例——家长会类：
{"title":"期末家长会上没讲大道理，家长都记笔记了","description":"期末家长会最关键的是帮家长理清一件事：成绩下滑不等于态度差，很可能是方法没跟上难度变化。\n语文：阅读从「找答案」变成「理解分析」，作文从「写完整」变成「写出重点」。\n数学：计算还是基础，但应用题开始需要读题建模，不是多练就能过。\n这个转变讲清楚，家长在家怎么配合就有方向了，焦虑也会少一半。","tags":["#家长会","#期末家长会","#班主任","#小学班主任","#家校共育","#班主任工作"]}`;
