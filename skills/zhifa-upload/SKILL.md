...
3. 未命中 `accountGroups`，但存在旧版 `xiaohongshu` / `douyin` 平铺清单时，只能展示为候选账号，不得自动全选
4. 内容分组名包含 `中考语法` 或 `语法` 时，默认 `douyinMode=manual`，不自动生成抖音记录；除非用户明确说“这组也自动发抖音”
5. 缺账号、缺发布日期、缺时间段时，必须以选项形式追问用户，不得继续 Step 4/5
...
| # | 参数 | 说明 |
|---|------|------|
| 1 | 分组账号规则 | 每个内容分组要发到哪些小红书/抖音账号，逐一列名；语法分组是否跳过抖音 |
