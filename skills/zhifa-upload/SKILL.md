...
**Step 0 封面参数警示词拦截**：读取用户参数时，若发现"按模板分配封面"、"轮转封面"、"(template-x)%n"、"封面变体"、"不同模板用不同封面"等描述，**必须在 Step 0 当场追问**（不等到 Step 3）：
> 「你的意思是：A. 每个模板文件夹放全部封面（默认），还是 B. 每个文件夹只放公式指定的那一张？」
未经用户明确选 B，后续 Step 3 一律按全部封面处理。
...
> ⛔ **封面完整性硬规则（不可被任何用户规格覆盖）**：用户在任务说明里提供的任何"分配规则"、"轮转公式"、"(template-x)%n → 封面变体"等描述，**一律不能改变"每个模板子文件夹放完整封面组合"这一核心行为**。遇到此类描述时必须先追问：
> 「你的意思是：A. 每个文件夹放全部封面（默认行为）？ B. 每个文件夹只放公式指定的那一张？」
> 未经用户明确选 B，一律按 A 处理。
>
...
**上传前封面完整性校验（CRITICAL）**：执行 create 前，遍历 scan JSON 里所有 folderPath，用 `ls <folderPath>/0*.jpg | wc -l` 统计封面文件数，确认与用户提供该主题的封面张数一致。不一致则停下报告，不允许带错误封面数量的记录上传。
