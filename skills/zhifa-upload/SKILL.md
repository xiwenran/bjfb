...
执行 Step 4 前，主会话**必须**先用 `sed -n '8,185p' ~/zhifa/src/ai-writer.js` 把 SYSTEM_PROMPT 字面文本提取出来，把内容**完整粘贴**到本步骤的工作上下文里（公式 1-8 + 示例全部）。不允许只写"请参考路径"——主会话自己也好、派子代理也好，所有用到这套规则的位置都必须看到字面文本。
...
2. 标题质量：字数 10-20 字？同主题不同笔记角度是否不同？有无年级词 / 缩写课文名等违规？
