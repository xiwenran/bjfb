...
**发布状态按用户意图设置**：上传时发布状态留空（知发设计），但 Step 6.8 后处理会根据用户意图补设——用户说"发布/排期"自动设「待发布」，用户只说"上传/导入"则留空。
...
| 发布状态 | 上传时留空，**Step 6.8 后处理根据用户意图设置**（说"发布"→待发布，说"上传"→留空） |
...
- 内容类型（家长会用公式 5-8 / 课文用公式 1-4）
- SYSTEM_PROMPT 路径：`~/zhifa/src/ai-writer.js`（子代理自行读取）
- 约束：每篇标题 10-20 字，最多 1 个标点，同主题不同篇必须角度不同，不允许重复
...
主会话展示所有标题/标签，等用户说「确认」后才继续。确认后主会话将标题/标签合并写入 `/tmp/zhifa_records.json`。
...
**② 设置发布状态**：用户说「发布 / 排期发布」→ 自动设「待发布」；用户只说「上传 / 建档」→ 留空。
...
for (const rid of recordIds) {
  const fields = { '笔记主题': '' };
  if (hasXhsAccount) fields['小红书发布状态'] = '待发布';
  if (hasDouyinAccount) fields['抖音发布状态'] = '待发布';
