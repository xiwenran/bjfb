...
- `build-records`：基于扫描结果、调度结果和文案映射生成 records，并把时间窗展开成随机具体发布时间；随机窗口必须传 `--seed`，保证同批排期可复现
...
`plan.json` 必须显式包含：
- `accountGroups`：用户确认后的小红书店铺分组账号池
- `douyinAccounts`：用户确认后的独立抖音账号池
...
执行 Step 5 前，主会话**必须**先用 `sed -n '8,69p' ~/zhifa/src/ai-writer.js` 把 SYSTEM_PROMPT 字面文本提取出来（2026-07 已重构为原则驱动+搜索流量优先，废弃旧的双层结构/钩子菜单/公式体系），把内容**完整粘贴**到本步骤的工作上下文和子代理派单里（核心目标 + 三条原则 + 正文写法 + 标签写法 + 硬边界 + 输出格式示例全部）。不允许只写"请参考路径"或"子代理自行读取"。
