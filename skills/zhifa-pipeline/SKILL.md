...
| 标题结构 | 「搜索词层+钩子层」双层结构（权威定义见 ai-writer.js SYSTEM_PROMPT）；公式一~四为普通课文/公开课选材参考，公式五~十为授课课件/复习课件/家长会/主题班会/班级管理/资料合集选材参考，公式十一~十四为无课文名场景选材参考，钩子层最终收敛到封闭菜单①~⑧ |
| 标题字数/emoji | 16-20 字为主，硬边界 10-20 字（emoji 计1字，ai-writer 硬约束）；恰好 1 个白名单 emoji（📌🔥✅💡✨📝） |
| 标签数量 | 小红书 ≤10 / 抖音 ≤5 |
| 笔记主题（topic 字段） | **课文类**：填课文名（如"《背影》"）；**家长会/班会/非课文类**：留空，records JSON 中设 `"topic": ""`，noteKey 仍保留完整路径。**上传后 Step 6.8 强制清空**，防止调度器 AI 写作自动覆盖标题 |
| 描述（description） | 必须撰写，禁止留空；3-5 行换行排版，行首白名单 emoji，总字数 50-150 字 |
...
执行 Step 5 前，主会话**必须**先用 `sed -n '8,312p' ~/zhifa/src/ai-writer.js` 把 SYSTEM_PROMPT 字面文本提取出来，把内容**完整粘贴**到本步骤的工作上下文和子代理派单里（标题双层结构 + 钩子菜单①~⑧ + 公式一~十四 + 正文排版规则 + 输出格式示例全部）。不允许只写"请参考路径"或"子代理自行读取"。
...
- 内容类型（普通课文用公式一~四选材；授课课件用公式五；复习课件用公式六；家长会用公式七或十一~十四；主题班会用公式八；班级管理用公式九；资料合集用公式十；无课文名场景直接用公式十一~十四）
- SYSTEM_PROMPT 字面文本：粘贴 `~/zhifa/src/ai-writer.js` 中 `SYSTEM_PROMPT` 的完整内容，不只给路径
- 约束：标题按「搜索词层+钩子层」双层结构组装，16-20字为主/硬边界10-20字，恰好1个白名单emoji，最多1个标点，同主题不同篇必须角度不同、钩子同批最多用2次且不相邻；正文必须撰写（3-5行换行排版，行首白名单emoji，50-150字，禁止留空）；课件类标题必须写"老师动作 + 学生/家长/班级结果"，禁止写成"课堂可用/重点清楚/老师可参考"这类资料说明词

子代理产出格式（JSON，字段与 ai-writer.js 输出格式一致）：
```json
{
  "英语成绩不是突然掉的，而是先松的（版本一）/1": {
    "title": "📌英语掉分早有信号，家长会上讲透了",
    "description": "📌这学期英语成绩变化梳理\n📝从平时作业和小测中找到的早期信号\n✅家长会上用具体例子讲清楚原因\n🔥提前发现问题，能帮孩子少走弯路",
    "hook": "⑦摸底考命题参考",
    "charCount": 18,
    "violations": [],
    "tags": ["#家长会", "#班主任", "..."]
...
主会话展示所有标题/正文/标签，等用户说「确认」后才继续。预览不得只展示标题，必须包含「公式/钩子 / 字数 / 违规项」三列，正文单独展示；违规项为 `无` 才能进入确认。确认后主会话将标题/正文/标签合并写入 `/tmp/zhifa_records.json`。

```
📋 内容预览（请确认后说「确认」继续）

| noteKey | 标题 | 公式/钩子 | 字数 | 违规项 | 标签 |
|---|---|---|---:|---|---|
| 主题A/1 | …… | ⑥一页讲透省时间 | 18 | 无 | #主题A #教学设计 |
正文（主题A/1）：📌…\n📝…\n✅…\n🔥…
```
...
`/api/ai-writing/generate` 服务端已内置机械校验（`validateGenerated`）+ 1 次自动重试：不合规会先反馈违规项重新生成一次，重试后仍不合规则直接抛错，不会静默放行不合规内容。fallback 成功返回 `title/description/tags` 后，主会话仍需按 SYSTEM_PROMPT 复核一遍 `hook/charCount/violations` 是否自洽，且违规项必须为 `无` 才能进入预览确认；不能手填一列后跳过复核。fallback 报错时，如实告知用户失败原因，不得手动编造内容绕过校验。
...
2. 标题质量：是否为「搜索词层+钩子层」双层结构？字数 16-20字为主/硬边界10-20字？恰好1个白名单emoji？同主题不同笔记角度是否不同？课件类标题是否写出"老师动作 + 对象结果"，而不是资料说明词？
2.5 正文质量：是否撰写（非空）？是否 3-5 行换行排版、行首白名单emoji、总字数 50-150？钩子是否来自封闭菜单①~⑧？
...
2. 标题：字数 16-20字为主/硬边界10-20字？恰好1个白名单emoji？同主题不同笔记标题角度是否不同？有无重复？
2.5 正文：是否撰写（非空）？3-5 行换行排版、行首白名单emoji、总字数 50-150？
3. 标签：小红书 ≤10 个？格式正确？
