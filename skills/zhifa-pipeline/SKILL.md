...
- `scan-many`：合并多主题扫描结果
- `materialize-covers`：把主题根目录或上级目录中的主序号 `0*` 封面批量下发到模板目录；如果没有任何 `0*` 封面，默认该批图文素材不需要单独补封面
- `schedule`：支持 `accounts + timeSlots` 和 `accounts + timeWindows`；先走同主题检查和一次性确认，再由服务端按固定 `seed` 分层随机，同批分钟唯一，同账号固定至少间隔 361 分钟
- `build-records`：基于 scan/schedule/content 生成 records，直接使用服务端具体分钟并透传 constraints，不二次随机；兼容 `--seed`，传入时必须与 schedule 一致
- `create`：自适应分批上传 records；真实上传必须保存完整 output，未传 `--output` 时脚本会自动写 `/tmp/zhifa_upload_create_results_<YYYYMMDD-HHMMSS>.json`
- `postprocess`：上传成功后清空 `笔记主题`，并按实际平台设置 `待处理`；大批量必须用 `--batch-size` 分批后处理
- `summarize-postprocess`：汇总一个或多个后处理结果，核对 updated、状态、账号和空 topic
- `export-preview`：从 records JSON 导出可搜索 `md/csv`，供上传前/上传后审查复用
- `archive`：按调度结果归档已安排 / 未安排

要求：
- 不再使用会话内临时脚本或手工 JSON 拼接替代这些共享步骤
- `zhifa-pipeline` 与 `zhifa-upload` 只允许在入口和停顿点上不同，底层执行层必须收敛
- PPT → 融景合成完成后，一旦进入“笔记上传”阶段，必须切到同一套 `scan-many → materialize-covers（仅有 0* 封面时） → schedule → build-records → create → postprocess → archive` 链路
- PPT 合成完成后的排期和上传规则以 `zhifa-upload` 为唯一规范；本 Skill 只复用入口和停顿点，不复制或另写排期算法

## Token 节制要求（2026-06-15 起）

进入**上传前冷眼审查（Step 5.5）**和**上传后汇报（Step 8）**前，优先调用 `token-summary` 获取摘要，主会话只读摘要 JSON，不直接读完整 `records.json` / 上传结果 JSON：
...
- **不得为节省 token 跳过 Step 5.5 / Step 6.8 / Step 8 等 CRITICAL 冷眼审查节点**
...
输出当前小红书账号池、抖音账号池，以及 `accountGroups`（仅小红书店铺组）映射。店铺分组是发布策略，不改变素材目录结构；小红书店铺组和抖音账号池分开配置，用户仍只需要整理 `导入根目录 / 内容分组 / 笔记或PPT主题`。
...
用户在 CHECKLIST 里已明确列了店铺分组账号规则时，以用户列的为准，跳过自动加载。账号变更时（加/减账号，或修改小红书店铺池 / 抖音账号池），任务完成后顺手更新 `accounts.json`。

**店铺分组优先级**：
1. 用户本次 CHECKLIST 明确写出的账号或店铺组规则，优先级最高
2. `accounts.json.accountGroups[小红书店铺组]` 命中时，作为默认推荐
3. 未命中时，只展示候选店铺组并追问，不得自动全选
4. 未命中分组配置时，只能把旧版平台平铺账号清单展示为候选，不得自动全选
5. 小红书店铺组只用于小红书；抖音使用独立抖音账号池，同一批素材可同步安排到抖音，并按独立账号池随机或轮转分配；若用户明确说某平台不发，再单独留空
6. 缺账号、缺店铺组、起止日期或时间段时，必须以选项形式追问用户，不能继续调度矩阵生成
...
| `zhifa/scripts/skill_upload.py materialize-covers` | 命令行 | 把主题根目录或上级目录的 `0*` 封面复制到每个模板目录；下发后必须重新 `scan-many`；没有 `0*` 时默认无单独封面继续 |
| `zhifa/scripts/skill_upload.py schedule` | 命令行 | 输入店铺分组、覆盖策略、日期时段等计划 → 返回调度矩阵；替代直接散调用 `/api/import/schedule` |
| `zhifa/scripts/skill_upload.py build-records` | 命令行 | 基于 scan/schedule/content 生成 records JSON，原样使用 schedule 的具体分钟并透传 constraints |
| `POST /api/ai-writing/generate` | HTTP | AI 生成标题/正文/标签（主会话手写失败时的 fallback） |
...
| 标题结构 | 搜索关键词为主体、自然表达（权威定义见 ai-writer.js SYSTEM_PROMPT 三原则：真实/搜索优先自然表达/每篇不同），不预设内容分类清单、不套用固定句式公式 |
| 标题字数/emoji | 8-20 字（全字符计数，含《》和emoji，ai-writer 硬约束）；emoji 可用可不用，最多 1 个 |
| 标签数量 | 小红书 ≤10 / 抖音 ≤5 |
| 笔记主题（topic 字段） | **课文类**：填课文名（如"《背影》"）；**家长会/班会/非课文类**：留空，records JSON 中设 `"topic": ""`，noteKey 仍保留完整路径。**上传后 Step 6.8 强制清空**，防止调度器 AI 写作自动覆盖标题 |
| 描述（description） | 必须撰写，禁止留空；首行搜索词展开句+中间逐行列内容，末行可选（无内容点直接收尾）；禁写「图片即为完整内容/完整内容见图/翻图查看」等完整性话术；无行数/字数/行首emoji硬性要求 |
| AI 写作 | 主会话先按 SYSTEM_PROMPT 原则手写 → 失败时 fallback 调 `/api/ai-writing/generate` |
...
从排期开始完整复用 `zhifa-upload`：排期前确认账号、账号→店铺组映射、日期、时间窗、固定 `seed` 和覆盖策略；“9 点左右”按 `08:00—12:00`。`plan.json.accountGroups` 唯一格式为 `{账号名: 店铺组名}`。先做同主题检查并一次性询问，再生成 schedule；旧记录忽略，主题索引、飞书或发布历史异常立即停止。

同平台同账号相邻笔记固定至少间隔 361 分钟，同批分钟全局唯一，服务端负责分层随机；Python 只透传结果。具体冲突判断、确认 fingerprint、三种选择和 constraints 契约只在 `zhifa-upload` 维护，本 Skill 不复制算法。

**跨平台素材复用**：小红书和抖音是独立发布平台，同一篇素材笔记可以各自生成一条发布记录；排期器只禁止同一平台内重复使用同一 `noteKey`，不得把小红书已用过的素材全局排除给抖音。典型场景：某小红书店铺组有 8 篇真实笔记，小红书 4 个账号 × 2 天 = 8 条；抖音使用独立 8 个账号池，也应能同步排满对应抖音记录。
...
若封面来自主题根目录/上级目录，或扫描结果提示模板目录缺主序号 0 封面，先检查目录链路是否存在 `0*` 封面。存在时执行：
...
后续步骤使用 `<scan_after_covers.json>`，不要继续引用补封面前的旧扫描文件。若目录链路里完全没有 `0*` 封面，默认该批图文素材不需要单独补封面，继续使用原扫描结果；不要停下追问，也不要移动非 `0*` 图片进去冒充封面。
...
- `plan.json`：由用户确认后的小红书店铺分组账号规则、独立抖音账号池、覆盖策略、日期、时间窗、每账号篇数构造
- `accounts`：必须按用户确认的小红书店铺分组展开到小红书账号池，并按独立抖音账号池展开抖音账号；同一批素材可同步安排到抖音
- `coverageStrategy`：必须来自用户显式确认
- `timeWindows` / `timeSlots`：允许使用日常时间窗口径；`schedule` 服务端展开成具体分钟，`build-records` 不再改时间

返回 `{schedule, unscheduled, stats, constraints}`：`schedule` 每项 `{topic, noteKey, platform, account, publishTime}`，其中 `publishTime` 已是服务端生成的 `YYYY-MM-DD HH:MM` 具体分钟；若仍返回时间窗字符串，视为排期失败并停止，禁止交给 `build-records` 二次处理。
...
### Step 5：生成标题/正文/标签 → 等用户确认（必停点）

**必须停在这一步等用户确认，然后才能继续 Step 6。**

**完整写作规范（CRITICAL — 字面注入，不许只写路径）**：

执行 Step 5 前，主会话**必须**先用 `sed -n '/^const SYSTEM_PROMPT/,/;$/p' ~/zhifa/src/ai-writer.js` 把 SYSTEM_PROMPT 字面文本提取出来（按标记提取，不依赖行号），把内容**完整粘贴**到本步骤的工作上下文和子代理派单里（核心目标 + 三条原则 + 正文写法 + 正文硬边界 + 标签写法 + 硬边界 + 输出格式示例全部）。不允许只写"请参考路径"或"子代理自行读取"。

生成标题前必须向用户明确写出：`标题规则来源：已读取 ~/zhifa/src/ai-writer.js 的 SYSTEM_PROMPT`。没写出这句，视为标题规则未进入本轮工作上下文。

优先派文本子代理生成所有笔记的标题、正文和标签。Codex 端按 Echo 规则声明 `子代理：文本生成 → model: gpt-5.4-mini`；Claude 端按对应模型规则执行：

派单内容：
- 所有笔记的 noteKey 列表（含模板编号）
- 内容类型：先按语义判断这份资料是什么（课件/教学设计/知识点总结/家长会/班会……不预设分类清单）
- SYSTEM_PROMPT 字面文本：粘贴 `~/zhifa/src/ai-writer.js` 中 `SYSTEM_PROMPT` 的完整内容，不只给路径
- 约束：标题以搜索关键词为主体、自然流畅表达，字数 8-20字（全字符计数），emoji 可用可不用最多1个；同主题不同篇必须角度不同、不套用同一句式；正文必须撰写（首行搜索词展开句+中间逐行列内容，末行可选、无内容点直接收尾，禁止留空）；正文和标题都禁写「图片即为完整内容/完整内容见图/内容按图整理/翻图查看/完整版」等完整性承诺话术（发布图只是部分素材）；标题只能用主题和图片文件名里真实存在的信息，不得编造属性

子代理产出格式（JSON，字段与 ai-writer.js 输出格式一致，仅 title/description/tags 三字段）：
```json
{
  "英语成绩不是突然掉的，而是先松的（版本一）/1": {
    "title": "初二英语期中家长会：成绩变化信号梳理",
    "description": "初二英语期中考试成绩变化情况分析\n从平时作业和小测中找到的早期信号\n家长会上用具体例子讲清楚原因\n提前发现问题，能帮孩子少走弯路",
    "tags": ["#家长会", "#班主任", "..."]
  }
}
```

主会话展示所有标题/正文/标签，等用户说「确认」后才继续。预览不得只展示标题，必须包含「标题 / 字数 / 违规项」三列，正文单独展示；违规项为 `无` 才能进入确认。确认后主会话将标题/正文/标签合并写入 `/tmp/zhifa_records.json`。

```
📋 内容预览（请确认后说「确认」继续）

| noteKey | 标题 | 字数 | 违规项 | 标签 |
|---|---|---:|---|---|
| 主题A/1 | …… | 18 | 无 | #主题A #教学设计 |
...
`/api/ai-writing/generate` 服务端已内置机械校验（`validateGenerated`）+ 1 次自动重试：不合规会先反馈违规项重新生成一次，重试后仍不合规则直接抛错，不会静默放行不合规内容。fallback 成功返回 `title/description/tags` 后，主会话仍需按 SYSTEM_PROMPT 硬边界复核一遍字数/标签数量/违规项是否自洽，且违规项必须为 `无` 才能进入预览确认；不能手填一列后跳过复核。fallback 报错时，如实告知用户失败原因，不得手动编造内容绕过校验。
...
**dry-run 语义边界**：`--dry-run` 只代表上述 5 项结构校验通过；它不检查标题是否符合 SYSTEM_PROMPT 三原则，也不判断标题吸引力。不得把 dry-run 通过表述为「标题合格」或「内容质量通过」。
...
审查前先导出可搜索预览，便于按账号、时间、noteKey、标题抽查：
...
2. 标题质量：是否以搜索关键词为主体、自然流畅表达（见 SYSTEM_PROMPT 三原则）？字数 8-20字？emoji 是否≤1个？同主题不同笔记角度是否不同？有无编造属性等违规？
...
真实上传必须保留完整结果文件。若漏传 `--output`，脚本会自动写入 `/tmp/zhifa_upload_create_results_<YYYYMMDD-HHMMSS>.json` 并打印路径；后续 Step 6.5、Step 6.8、Step 8 都必须引用该结果文件，不得只看终端摘要。
...
`create` 必须输出完整上传结果 JSON，供 Step 6.5、Step 6.8 和 Step 8 复用；不得再临时手写脚本调用 `/api/import/create-records` 只为保存返回值。`postprocess` 必须按 `--batch-size` 分批处理大批量，默认 50 条一批，避免 95 条一次请求超时；必须输出结果文件，并用 `summarize-postprocess` 汇总 updated、rows、平台状态、topic 空值和账号计数。FeishuClient 直连只允许作为共享命令不可用时的降级兜底，并必须向用户说明。
...
上传结果：create 打印或显式指定的上传结果 JSON + retry 结果 JSON（如有）
后处理结果：postprocess 结果 JSON + summarize-postprocess 汇总 stdout/JSON
导入日志：~/Library/Caches/Zhifa/logs/import-debug.log

审查项（读 JSON 文件 + 导入日志，每项给 ✅/❌/⚠️）：
1. 模板多样性：每个账号使用的模板种类是否充足？是否存在某账号模板过度集中？
2. 标题：字数 8-20字？emoji 是否≤1个？同主题不同笔记标题角度是否不同？有无重复或编造属性？
2.5 正文：是否撰写（非空）？是否按首行搜索词展开/中间列内容的结构写（末行可选，不强制）？有无「图片即为完整内容/完整内容见图/翻图查看/按图整理」等完整性话术（有即打回）？
3. 标签：小红书 ≤10 个？格式正确？
4. 调度合规：同批分钟是否唯一？同平台同账号相邻笔记是否至少间隔 361 分钟？同主题冲突决定是否与本轮 fingerprint 一致？
