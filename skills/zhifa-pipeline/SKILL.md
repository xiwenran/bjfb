...
| 描述（description） | 必须撰写，禁止留空；首行搜索词展开句+中间逐行列内容+末行自然收住，无行数/字数/行首emoji硬性要求 |
...
- 约束：标题以搜索关键词为主体、自然流畅表达，字数 8-20字（全字符计数），emoji 可用可不用最多1个；同主题不同篇必须角度不同、不套用同一句式；正文必须撰写（首行搜索词展开句+中间逐行列内容+末行自然收住，禁止留空）；标题只能用主题和图片文件名里真实存在的信息，不得编造属性
...
2.5 正文质量：是否撰写（非空）？是否按首行搜索词展开/中间列内容/末行收住的结构写？
...
2.5 正文：是否撰写（非空）？是否按首行搜索词展开/中间列内容/末行收住的结构写？
3. 标签：小红书 ≤10 个？格式正确？
